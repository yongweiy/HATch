...
let create_worklist ~init_width = Pairing_heap.create ~min_size:20 ~cmp:(fun c1 c2 ->
  (* Strictly prioritize smaller width over smaller steps *)
  match Int.compare c1.width c2.width with
  | 0 -> Int.compare c1.steps c2.steps  (* If widths are equal, prefer fewer steps *)
  | cmp -> cmp  (* Otherwise, prefer smaller width *)
)
    (* (fun c1 c2 -> *)
