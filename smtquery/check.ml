open Sig
module F (P : PROP) (MT : MINTERM) (R : REGEX) = struct
  open Z3
  open Solver
  open Goal
  (* open Z3aux *)
  ...
  let smt_neg_and_solve ctx vc =
    ...
    (* in *)
    let assertions =
      List.map (P.to_z3 ctx) [ P.get_asm vc; P.mk_not vc ]
    in
    (* let () = *)
  ...
  let mk_q_version1 ctx r1 r2 =
    let encoding, r1, r2 = R.to_z3_two_reg ctx (r1, r2) in
  ...
  let mk_q_version2 ctx r1 r2 =
    ...
    let () =
      ...
      Printf.printf "R2:\n%s\n" (R.reg_to_string r2)
    in
    let r12 = R.(mk_and r1 @@ mk_not r2) in
    let encoding, r = R.to_z3_one_reg ctx r12 in
    let encoded_size = R.get_size encoding r12 in
    let sequence = Expr.mk_const_s ctx sequence_name (Seq.mk_string_sort ctx) in
  ...
  let layout_counterexample mt_list =
    match mt_list with
    | [] -> "ϵ"
    | _ -> List.split_by ";" MT.mt_to_string mt_list

  let inclusion_query ctx r1 r2 =
    (* let open Sugar in *)
    let encoding, encoded_size, qs = mk_q_version2 ctx r1 r2 in
    let num_inclusion_alphabet = R.RegZ3.get_cardinal encoding in
    let _ =
    ...
    let res =
      ...
      | Some model ->
          ...
          in
          let mt_list = R.RegZ3.code_trace encoding str in
          (* ( MetaConfig.show_debug_queries @@ fun _ -> *)
