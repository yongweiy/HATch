...
module F (P : PROP) (MT : MINTERM) (R : REGEX with type mt = MT.mt) = struct
  module Check = Check.F (P) (MT) (R)
  ...
  let cached (cache : (P.prop, bool * int) Hashtbl.t) (check : P.prop -> bool)
      (vc : P.prop) =
  ...
  let check_sat_bool vc =
    ...
    let () =
      MetaConfig.show_debug_stat @@ fun _ ->
      Printf.printf "check_sat_bool: %f\n" runtime
    in
  ...
  let check_inclusion_bool (r1, r2) =
    let runtime, (size, res) =
      Sugar.clock (fun () -> check_inclusion (r1, r2))
    in
    let () =
