...
let weaken_with_query rctx (inferred_type : rty) (query_type : rty) : rty option
  ...
  try
    ...
    if Subtyping.is_bot_rty rctx result then None else Some result
  with _ -> None

let weaken_eff_with_query rctx (inferred_eff : monad) (query_eff : monad) :
    monad option =
  try
    let result = weaken_eff_type rctx query_eff inferred_eff in
    if Subtyping.is_bot_rty rctx result.ret.rty then None else Some result
  with _ -> None
