...
let check_eff_event opctx ctx { op; vs; v; phi } =
  ...
  let bindings = List.map (Coersion.Aux.force __FILE__ __LINE__) (v :: vs) in
...
let check opctx ctx sevent =
  ...
  | GuardEvent phi ->
      let phi = type_check_qualifier opctx ctx phi in
      GuardEvent phi
  | EffEvent ev -> EffEvent (check_eff_event opctx ctx ev)
